...
from agentic_pdm_data_cleaning.experiment.benchmark_executor import BenchmarkExecutor
from agentic_pdm_data_cleaning.experiment.environment_generator import EnvironmentDataGeneration
...
def main() -> int:
    ...
    argparser = argparse.ArgumentParser(
        description=(
            "Set up experiment environment with synthetic data, database, "
            "preprocessing, and benchmark execution"
        )
    )
    ...
    try:
        ...
        print(f"Error loading benchmark config: {error}")
        traceback.print_exc()
        return 1

    fleet_id = benchmark_config.get("global_params", {}).get(
        "fleet_id", benchmark_config.get("fleet_id", "default_fleet")
    )

    fs = FileSystem()
    try:
        load_config(fs.fleet_config_file(fleet_id))
    except Exception as error:
        print(f"Error loading fleet config: {error}")
        traceback.print_exc()
        return 1
    ...
    if not args.skip_data_generation:
        for run in range(base_runs):
            print(f"Setting up environment for benchmark {benchmark_id} run {run}...")
    ...
    if args.execute_experiment_evaluation:
        ...
        for experiment_name, experiment_config in experiments.items():
            ...
            evaluator.compute_aggregate_eval_metrics(
                input_jsonl=fs.get_common_eval_report_path(
                    benchmark_id=benchmark_id,
                    experiment_name=experiment_name,
                ),
                save_csv=fs.get_aggregate_eval_metrics_path(
                    benchmark_id=benchmark_id,
                    experiment_name=experiment_name,
                ),
            )
            evaluator.compute_aggregate_llm_usage(
                ...
                save_csv=fs.get_aggregate_usage_metrics_path(
                    benchmark_id=benchmark_id,
                    experiment_name=experiment_name,
                ),
            )
